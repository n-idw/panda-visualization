...
def make_cmp_plot_old(
    ...
    fig.savefig("{}.png".format(outname), dpi=300)
    ...
    ax.grid(False)
    fig.savefig("{}_ratio.pdf".format(outname), dpi=300)
    fig.savefig("{}_ratio.png".format(outname), dpi=300)
...
def make_cmp_plot(
    ...
    for array, legend in zip(arrays, legends):
        vals, bins, _ = ax.hist(
            array, **configs, color=cmap[i], linestyle=lmap[i], label=legend
        )
        vals_list.append(vals)
        i = i + 1
    ...
    ax.grid(False)
    fig.savefig("{}.pdf".format(outname), dpi=300)
    fig.savefig("{}.png".format(outname), dpi=300)
